...
- 접근 보호: `/login` 에서 이름을 입력하면 서명 쿠키 세션(30일)이 발급된다(`src/lib/auth/session.ts`, `src/proxy.ts`). `LOGIN_ALLOWED_IDS`(콤마 구분 이메일)에 있는 계정만 로그인되며, `APP_PASSWORD` 를 설정하면 비밀번호도 요구. 서명 키는 `AUTH_SECRET`. 크롤러는 `CRAWLER_API_KEY` 헤더로 통과.
