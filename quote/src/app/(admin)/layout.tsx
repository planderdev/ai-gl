...
import { currentUser, displayName } from "@/lib/auth/session";
    ...
    <>
      <header className="admin-header" id="adminHeader">
        ...
        <div className="admin-header__right">
          ...
          <div className="admin-user-chip"><span className="admin-user-chip__avatar">{displayName(user?.name ?? "관").slice(0, 1).toUpperCase()}</span><span title={user?.name}>{displayName(user?.name ?? "관리자")}</span></div>
