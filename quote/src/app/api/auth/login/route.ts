...
import { createSessionToken, passwordRequired, SESSION_COOKIE } from "@/lib/auth/session";
...
export async function POST(req: Request) {
  ...
  if (!name) return NextResponse.json({ error: "이름(아이디)을 입력하세요" }, { status: 422 });
...
export async function GET() { return NextResponse.json({ passwordRequired: passwordRequired() }); }
